...
using RTS.Simulation.Data;
using RTS.Simulation.Systems;
using RTS.Simulation.Core; // SimGameContext erişimi için
...
public class GameVisualizer : MonoBehaviour
{
    ...
    [Header("Görsel Ayarlar")]
    public Color Player2Color = new Color(1f, 0.8f, 0.2f);
    public Color DamageFlashColor = Color.red;
    ...
    // --- TAKİP VERİLERİ ---
    private Dictionary<int, GameObject> _spawnedObjects = new Dictionary<int, GameObject>();
    private Dictionary<int, int> _lastKnownHealth = new Dictionary<int, int>();
    private Dictionary<int, float> _flashTimers = new Dictionary<int, float>();

    private GameObject _currentMapParent;

    // YENİ: Özel olarak atanmış dünya (Eğitim için)
    private SimWorldState _overrideWorld;

    // Runner tarafından çağrılır (Eğitim Modu)
    public void Initialize(SimWorldState world)
    {
        _overrideWorld = world;
        ResetVisuals();
        // Harita Update içinde çizilecek
    }

    void Update()
    {
        // 1. Hangi dünyayı çizeceğimize karar verelim
        // Eğer Initialize ile özel dünya verildiyse onu kullan, yoksa Global olana bak.
        SimWorldState world = _overrideWorld;

        if (world == null)
        {
            world = SimGameContext.ActiveWorld; // Eski sahnelerin çalışmaya devam etmesini sağlayan satır
        }

        if (world == null) return;

        // Harita henüz çizilmediyse çiz
        if (_currentMapParent == null) RegenerateMap(world.Map);
    ...
    // --- BURADAN AŞAĞISI SENİN ORİJİNAL KODLARIN (DEĞİŞMEDİ) ---
    // Sadece Sync fonksiyonlarını buraya kopyalıyoruz ki eksik kalmasın.
    ...
    void SyncResource(SimResourceData r, GameObject prefab)
    {
        ...
        float percent = (float)r.AmountRemaining / 250f;
        float alpha = Mathf.Max(0.3f, percent);
        var sr = obj.GetComponent<SpriteRenderer>();
        if (sr) sr.color = new Color(1f, 1f, 1f, alpha);
    }
    ...
    void UpdatePosition(GameObject obj, int2 gridPos, bool smooth)
    {
        ...
        if (smooth && Vector3.Distance(obj.transform.position, targetPos) < 3.0f)
            obj.transform.position = Vector3.Lerp(obj.transform.position, targetPos, Time.deltaTime * 10f);
        else
            obj.transform.position = targetPos;
    }
    ...
    GameObject GetOrCreateObject(int id, int2 gridPos, GameObject prefab)
    {
        ...
        if (!_spawnedObjects.ContainsKey(id))
        {
            ...
            // SimEntityVisual ekle (Seçim sistemi için gerekebilir)
            var visualID = newObj.GetComponent<SimEntityVisual>();
            if (visualID == null) visualID = newObj.AddComponent<SimEntityVisual>();
            visualID.ID = id;
    ...
    public void RegenerateMap(SimMapData map)
    {
        ...
        _currentMapParent = new GameObject("Map_Tiles");
        _currentMapParent.transform.SetParent(this.transform);
        _currentMapParent.transform.localPosition = Vector3.zero;
    ...
    {
        if (_currentMapParent != null) Destroy(_currentMapParent);
        foreach (var obj in _spawnedObjects.Values) if (obj != null) Destroy(obj);
        _spawnedObjects.Clear();
        _lastKnownHealth.Clear();
        _flashTimers.Clear();
        _currentMapParent = null;
